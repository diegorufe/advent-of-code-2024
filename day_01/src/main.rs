...
fn convert_to_vectors(input_file_name: &str)-> (Vec<i32>, Vec<i32>){
    ...
    for line in reader.lines() {
        let values : Vec<i32>  = line.expect("Error read line").split_whitespace().map(|s| s.parse().unwrap()).collect();
...
fn  history_example_part_one(){
    ...
    let (location_ids_a, location_ids_b): (Vec<i32>, Vec<i32>) = convert_to_vectors("./src/history_example_part_one_data.txt");
    let distance = calculate_distance(location_ids_a, location_ids_b);
    let duration = start.elapsed();
    print!("History example part one. Distance {}. Time: {:?} \n", distance, duration)
}

fn  history_part_one(){
    let start = Instant::now();
    let (location_ids_a, location_ids_b): (Vec<i32>, Vec<i32>) = convert_to_vectors("./src/history_part_one_data.txt");
    let distance = calculate_distance(location_ids_a, location_ids_b);
    let duration = start.elapsed();
    print!("History part one. Distance {}. Time: {:?}  \n", distance, duration)
}

fn  history_example_part_two(){
    let start = Instant::now();
    let (location_ids_a, location_ids_b): (Vec<i32>, Vec<i32>) = convert_to_vectors("./src/history_example_part_two_data.txt");
    let score = calculate_score(location_ids_a, location_ids_b);
    let duration = start.elapsed();
    print!("History example part two. Score {}. Time: {:?} \n", score, duration)
}

fn  history_part_two(){
    let start = Instant::now();
    let (location_ids_a, location_ids_b): (Vec<i32>, Vec<i32>) = convert_to_vectors("./src/history_part_two_data.txt");
    let score = calculate_score(location_ids_a, location_ids_b);
    let duration = start.elapsed();
